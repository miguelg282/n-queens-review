...
window.findSolution = function (row, n, board, validator, callback) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    //recurse into remaining problem if no rook conflicts
    if (!board[validator]()) {
      var result = findSolution(row + 1, n, board, validator, callback);
      if ( result ){
        return result;//Eject, stops all remaining recursion.
      }
    }
    //unplace a piece
    board.togglePiece(row, i);
...
//based off the decision tree for all possible moves for each row.
